...
const authService = {
  ...
  async resendActivation(email) {
    const res = await api.post("/auth/resendActivation", null, {
      params: { email },
    });
  ...
  async sendSetPasswordLink(email) {
    ...
    return {
      message: "If eligible, a set-password link has been sent to your email.",
    };
  },
  ...
  async setPassword({ token, newPassword }) {
    ...
    return {
      message:
        "Your password has been set. You can sign in with email/password now.",
    };
  },
};
