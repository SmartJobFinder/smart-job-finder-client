...
export default function NotificationBell({ className = "" }) {
  ...
  const getKey = it =>
    it?.id ?? it?.clientId ?? it?.uuid ?? it?.notiId ?? it?.providerId ?? null;
  ...
  const mergeUnique = (prev, incoming, prepend = false) => {
    ...
    (incoming || []).forEach(it => {
      ...
      if (!knownIdsRef.current.has(key)) {
        ...
        const idx = next.findIndex(x => (x._k ?? String(x.id)) === key);
        if (idx >= 0) next[idx] = { ...next[idx], ...it, id: key, _k: key };
      }
    });
    return next;
  };
      ...
      onConnect: () => {
        ...
        subRef.current = client.subscribe(SUB_DEST, frame => {
          ...
          if (notif) {
            ...
            if (openRef.current) {
              setItems(prev => mergeUnique(prev, [notif], true));
            }
          }
      ...
      <PopoverContent align="end" className="p-0 w-96">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b">
          <div className="text-sm font-semibold">
            {t`Notifications`} {count > 0 ? `(${count} Unread)` : ""}
          </div>
          <Button
            variant="ghost"
            size="sm"
