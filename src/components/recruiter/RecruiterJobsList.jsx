...
import {
  searchJobsByCompany,
  buildDefaultFiltersFor,
} from "@/services/recruiterJobsService";
...
export default function RecruiterJobsList({ tab = "all" }) {
  ...
  const userCompanyId =
    user?.companyId || user?.company?.id || user?.company_id;
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      const res = await searchJobsByCompany({
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 text-sm text-muted-foreground flex items-center gap-2">
              ...
              <span className="truncate">
                {job.company?.company_name || job.company?.companyName}
              </span>
            </div>
          ...
          <div className="text-right shrink-0 text-xs text-muted-foreground space-y-1">
            <div className="flex items-center gap-1 justify-end">
              <Calendar className="w-3 h-3" /> {t`Posted`}: {job.date_post}
            </div>
            <div className="flex items-center gap-1 justify-end">
              <Calendar className="w-3 h-3" /> {t`Expired`}: {job.expired_date}
            </div>
            <div className="pt-2">
  ...
  const EmptyState = () => (
    <Card className="p-10 text-center">
      <div className="flex flex-col items-center space-y-4">
        ...
        <div>
          <div className="text-lg font-medium mb-2">{t`No jobs found`}</div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select value={sort} onValueChange={setSort}>
          ...
          <SelectContent>
            ...
            <SelectItem value="expiredDate,asc">{t`Expiring soon`}</SelectItem>
...
//         // Simulate API delay
//         setTimeout(() => {
//             const mockJobs = getJobsByStatus(tab);
//             setData({
//                 content: mockJobs,
//                 totalPages: 1,
