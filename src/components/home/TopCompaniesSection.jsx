...
import { Button } from "@/components/ui/button";
import { t } from "@/i18n/i18n";
// Import mock companies data instead of fetching from API
...
function normalizeCompany(c) {
    return {
        ...
        openPositions: c.job_count || 0,
        pro: !!c.isProCompany || !!c.proCompany,
    };
}
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            try {
                ...
                const items = mockCompaniesData;
                const normalized = items.map(normalizeCompany);
                const finalList = sortWithPro(normalized);
                setCompanies(finalList);
                setLoading(false);
            } catch (e) {
                setErr("Error loading mock data");
                setLoading(false);
            }
