...
// // Chuẩn hoá từ item: { job, saved, applied }
// function normalizeItem(job, savedJobs = [], appliedJobs = []) {
//     const j = job || {};
//     const companyName = j?.company?.company_name || "Unknown Company";
//     const logo = j?.company?.avatar || "/logo-placeholder.png";
//     const firstWorkType = j.work_type || "FULL-TIME";
...
//         boosted: !!j?.boosted,
//         boostLevel: Number(j?.boostLevel) || 0,
//         liked: savedJobs.includes(j.id),
//         applied: appliedJobs.includes(j.id),
//     };
// }
...
//     // Mock saved and applied job IDs (in a real app, these would come from the API)
//     const [savedJobIds, setSavedJobIds] = useState([1, 3]);
//     const [appliedJobIds, setAppliedJobIds] = useState([2]);
...
//                 // Get the first 12 jobs from the mock data
//                 const jobsToShow = jobs.slice(0, 12);

//                 // Normalize the job data
//                 const normalized = jobsToShow.map((job) =>
//                     normalizeItem(job, savedJobIds, appliedJobIds)
//                 );

//                 const finalList = sortWithBoost(normalized);
