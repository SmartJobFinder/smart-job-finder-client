...
export default function SetPasswordForm({
    ...
    useEffect(() => {
        if (!token) {
            toast.error(
                "Invalid or missing token. Please click the email link again."
            );
            onInvalidToken?.();
        }
    ...
    const onSubmit = async (e) => {
        ...
        try {
            await dispatch(
                setPasswordThunk({ token, newPassword: pw })
            ).unwrap();
            toast.success(
        ...
        <CardContent>
            {!token && (
                <div className="mb-4 flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">
                    ...
                    <span>
                        Invalid or missing link. Please open the set-password
            ...
            <form onSubmit={onSubmit} className="space-y-4">
                ...
                <p className="text-center text-xs text-gray-500">
                    You will still be able to sign in with Google after setting
                    a password.
                </p>
            </form>
        </CardContent>
