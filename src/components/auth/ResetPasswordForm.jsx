...
export default function ResetPasswordForm({
    ...
    const meetsAllRules =
        rules.len && rules.upper && rules.lower && rules.digit && rules.special;
    ...
    const onSubmit = async (e) => {
        ...
        try {
            await dispatch(
                resetPasswordThunk({ token, newPassword: pw })
            ).unwrap();
            setSuccess(true);
    ...
    const strengthLabel = pw.length
        ? strengthLabelMap[Math.max(0, rules.score - 1)]
        : "";
        ...
        <form onSubmit={onSubmit} className="space-y-5 max-w-md">
            {!token && (
                <div className="flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">
                    ...
                    <span>
                        Reset token is missing or invalid. Please request a new
                        link on the Forgot Password page.
                    </span>
                </div>
            )}
            ...
            {success && (
                <div className="flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 p-3 text-sm text-green-700">
                    ...
                    <span>
                        Password updated. Redirecting to login in{" "}
                        <b>{redirectIn}s</b>…
                    </span>
                </div>
            )}
