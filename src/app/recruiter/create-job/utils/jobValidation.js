...
const formatJobDataToText = formData => {
  ...
  if (formData.jobDescription) {
    const cleanDesc = formData.jobDescription.replace(/<[^>]*>/g, "").trim();
    if (cleanDesc) {
  ...
  try {
    ...
    if (aiResult.isScam) {
      ...
      if (aiResult.scamProb > 0.7) {
        ...
        issues.push("Some aspects of this job posting may raise concerns");
        ...
        suggestions.push("Consider reviewing job description for completeness");
        suggestions.push("Ensure all information is accurate and professional");
      }
    } else {
