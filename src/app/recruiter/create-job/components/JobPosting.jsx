...
export default function JobPostingForm() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!companyId) {
        toast.error("Cannot find company information. Please login again.");
        return false;
      }
      ...
      const buildLocationString = () => {
        ...
        if (formData.wardIds && formData.wardIds.length > 0) {
          ...
          if (selectedWards.length > 0) {
            const wardNames = selectedWards.map(ward => ward.name).join(", ");
      ...
      if (trustLabel === "SUSPICIOUS") {
        ...
        toast.success("Job created successfully with verified status!", {
          position: "top-center",
          autoClose: 5000,
        });
      } else {
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Fill in the details to create a job</p>
        </div>
