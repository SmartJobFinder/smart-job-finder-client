...
export default function InterviewJoinPage({ params }) {
  ...
  function handleApiReady(externalApi) {
    externalApi.addEventListener("conferenceFailed", err => {
      ...
      const reason = err?.name || (err && JSON.stringify(err)) || "unknown";
      if (
      ...
      <div className="p-4 bg-white rounded shadow">
        ...
        <div className="flex flex-col items-start gap-2 mt-5">
          ...
          <p className="max-w-xs text-sm text-gray-600">
            If the embedded version fails, please click this button and we’ll
            redirect you to Jitsi in a new tab.
          </p>
        </div>
      </div>
      ...
      {room ? (
        ...
        <div className="p-4 rounded bg-gray-50">
          <p className="text-sm text-gray-700">
            Meeting room not available (no embed). Use portal link instead:
          </p>
          {portal ? (
            <a
              href={portal}
            ...
            <p className="text-sm text-gray-500">No meeting URL provided.</p>
          )}
        </div>
      )}
