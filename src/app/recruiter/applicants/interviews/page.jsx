  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const cid =
          res?.id || res?.company_id || res?.companyId || res?.company?.id;
            ...
            <tbody>
              {isLoading || !canQuery ? (
                ...
                (data?.content || []).map(row => (
                  ...
                  >
                    <td className="px-6 py-3">{formatDT(row.scheduledAt)}</td>
                    <td className="px-6 py-3">
                      {row.jobTitle || `Job #${row.jobId}`}
                    </td>
                    <td className="px-6 py-3">
                      {row.candidateName || `User #${row.candidateId}`}
                    </td>
                    ...
                    <td className="px-6 py-3">
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Button
                          variant="secondary"
                          size="sm"
                          disabled={updating || row.status === "COMPLETED"}
                        ...
                        <Button
                          variant="destructive"
                          size="sm"
                          disabled={updating || row.status === "CANCELLED"}
                          onClick={() => onChangeStatus(row, "CANCELLED")}
                        >
                          {t`Cancel`}
                        </Button>
                      </div>
