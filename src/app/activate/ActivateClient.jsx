...
export default function ActivateClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setStatus("success");
        setMessage(
          res?.message || "Account activated successfully. You can now log in."
        );
      } catch (e) {
        const st = e?.status;
        ...
        if (st === 409) {
          setStatus("success");
          setMessage(
            dataMsg || "Your account is already activated. You can log in now."
          );
          return;
        }
        if (st === 410 || st === 422) {
          ...
          setMessage(
            dataMsg ||
              "Activation link is expired or invalid. Please request a new one."
          );
          return;
        }
        setStatus("failed");
  ...
  useEffect(() => {
    ...
    const id = setInterval(
      () => setRedirectCountdown(s => Math.max(0, s - 1)),
      1000
    );
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <ActivationResultCard
        ...
        resendProps={{
          ttlSec: TTL_SEC,
          fallbackCooldownSec: RESEND_COOLDOWN_SEC,
        }}
      />
    </div>
  );
