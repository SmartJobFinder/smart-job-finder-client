...
export default function ProfilePage() {
  ...
  const profileSectionData = getProfileSectionData(normalizedProfileData || {});
  ...
  const refreshSectionAfterCRUD = async sectionId => {
    try {
      const { data: freshData } = await getSectionItems(sectionId).unwrap();
    ...
    if (
      ...
      try {
        ...
        if (error.status === 401) {
          ...
          toast.error(
            "Failed to delete item: " + (error.data?.message || error.message)
          );
        }
      } finally {
        setIsDeleting(false);
    ...
    try {
      if (
        ...
        const updatedData = {
          ...
          title:
            currentSection.id === "personalDetail"
              ? newData.title || normalizedProfileData?.personalDetail?.title
              : normalizedProfileData?.personalDetail?.title,
          email:
            currentSection.id === "personalDetail"
              ? newData.email || normalizedProfileData?.personalDetail?.email
          ...
          phone:
            currentSection.id === "personalDetail"
              ? newData.phone || normalizedProfileData?.personalDetail?.phone
              : normalizedProfileData?.personalDetail?.phone,
          gender:
            currentSection.id === "personalDetail"
              ? newData.gender || normalizedProfileData?.personalDetail?.gender
              : normalizedProfileData?.personalDetail?.gender,
          personalLink:
            currentSection.id === "personalDetail"
              ? newData.personalLink ||
                normalizedProfileData?.personalDetail?.personalLink
              : normalizedProfileData?.personalDetail?.personalLink,
          aboutMe:
            currentSection.id === "aboutMe"
              ? newData.text
        ...
        if (editingItemIndex !== null) {
          const itemId =
            normalizedProfileData[currentSection.id]?.[editingItemIndex]?.id;

          if (itemId) {
            await updateSectionItem({
              section:
                sectionToEndpointMap[currentSection.id] || currentSection.id,
              itemId: itemId,
              data: newData,
            }).unwrap();
      ...
      if (error.status === 401) {
        ...
        toast.error(
          "Failed to save data: " + (error.data?.message || error.message)
        );
      }
    }
  };
      ...
      <div className="flex-1 w-full xl:max-w-[78%]">
        <div className="grid grid-cols-1 gap-4">
          {profileSectionData.map(section => {
            ...
            const hasContent = hasMeaningfulContent(section.id, sectionData);
        ...
        <>
          {currentSection.id === "personalDetail" ? (
            <PersonalDetailModal
              ...
              validationSchema={
                profileSectionConfigs[currentSection.id].validationSchema
              }
              initialData={normalizedProfileData?.personalDetail || {}}
              onSave={handleSave}
            ...
            <CandidateSkillModal
              ...
              existingSkills={normalizedProfileData?.candidateSkills || []}
              onSave={handleSave}
              onClose={handleCloseModal}
            />
          ) : (
