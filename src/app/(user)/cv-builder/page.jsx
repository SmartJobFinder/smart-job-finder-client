  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const parsed = detail?.content ? JSON.parse(detail.content) : emptyCv;
        if (!alive) return;
        setCv(parsed);
      ...
      <CvSection title={t`Education`}>
        {(cv.edu || []).length ? (
          <div className="space-y-3">
            {cv.edu.map(e => (
              <div key={e.id} className="border border-black rounded-lg p-3">
                <div className="flex justify-between gap-3">
                  <div className="font-semibold text-black">
                    {e.schoolName || "-"}
                  </div>
                  <div className="text-sm text-black">{e.duration || ""}</div>
                </div>
                {e.majors && (
                  <div className="text-sm text-black">{e.majors}</div>
                )}
                {e.degree && (
                  <div className="text-xs text-black mt-1">{e.degree}</div>
                )}
              </div>
            ))}
          </div>
        ...
        {(cv.experience || []).length ? (
          <div className="space-y-3">
            {cv.experience.map(exp => (
              <div key={exp.id} className="border border-black rounded-lg p-3">
                <div className="flex justify-between gap-3">
                  <div className="font-semibold text-black">
                    {exp.position || "-"}
                  </div>
                ...
                {exp.companyName && (
                  <div className="text-sm text-black">{exp.companyName}</div>
                )}
                {exp.description && (
            ...
            <div className="space-y-2">
              {cv.edu.map(e => (
                <div key={e.id} className="border border-black rounded-lg p-3">
                  ...
                  <div className="text-xs text-black">{e.duration || ""}</div>
                  {e.majors && (
                    <div className="text-sm mt-1 text-black">{e.majors}</div>
                  )}
                </div>
              ))}
            </div>
    ...
    <div className="space-y-6">
      ...
      <div className="text-center space-y-3 pb-4 border-b-2 border-black">
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 text-sm text-black">
          {cv.information.phone && <span>{cv.information.phone}</span>}
          {cv.information.email && <span>{cv.information.email}</span>}
          {cv.information.location && <span>{cv.information.location}</span>}
        </div>
      </div>
          ...
          {(cv.edu || []).length ? (
            cv.edu.map(e => (
              <div key={e.id} className="space-y-1">
                ...
                {e.majors && (
                  <div className="text-sm text-gray-700">{e.majors}</div>
                )}
                {e.degree && (
                  <div className="text-xs text-gray-600">{e.degree}</div>
                )}
              </div>
            ))
          ) : (
          ...
          {(cv.experience || []).length ? (
            cv.experience.map(exp => (
              <div key={exp.id} className="space-y-1">
                ...
                {exp.companyName && (
                  <div className="text-sm text-gray-700">{exp.companyName}</div>
                )}
                {exp.description && (
