...
const PersonalDetailModal = ({
  ...
  const [previewAvatar, setPreviewAvatar] = useState(initialData.avatar || "");
  ...
  const onSubmit = data => {
    ...
    config.fields.forEach(field => {
      if (field.type === "file" && data[field.key] && data[field.key][0]) {
        ...
        processedData[field.key] = format(data[field.key], "yyyy-MM-dd");
      }
    });
                  ...
                  <div className="w-full mt-1 text-xs text-center min-h-[1rem]">
                    {errors.avatar ? (
                      <span className="block max-w-[8rem] text-red-500 break-words">
                        {errors.avatar.message?.includes("5MB")
                          ? `${errors.avatar.message}. Current size: ${(
                              (watchedValues.avatar?.[0]?.size || 0) /
                              (1024 * 1024)
                            ).toFixed(2)} MB`
