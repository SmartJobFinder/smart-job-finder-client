...
export default function GenericModal({
  ...
  const handleYearChange = (key, year) => {
    const month = watchedValues[key] ? format(watchedValues[key], "MM") : "01";
    const newDate = parse(`${month}/01/${year}`, "MM/dd/yyyy", new Date());
    setValue(key, newDate, { shouldValidate: true });
  };
  ...
  const onSubmit = data => {
    ...
    config.fields.forEach(field => {
      if (field.type === "file" && data[field.key] && data[field.key][0]) {
    ...
    if (
      ...
      if (data.endDate && !isOngoing) {
        processedData.duration += ` - ${format(data.endDate, "MM/yyyy")}`;
      } else if (isOngoing) {
        processedData.duration += " - NOW";
      }
      delete processedData.startDate;
      delete processedData.endDate;
    } else if (sectionId === "education" || sectionId === "workExperience") {
            ...
            {config.fields.map(field => (
              <div key={field.key} className="flex flex-col gap-1">
                <label className="text-sm font-medium text-gray-700 capitalize">
                  {isDurationSection && field.key === "startDate"
                    ? "From"
                    : isDurationSection && field.key === "endDate"
                      ? "To"
                      : field.label}
                  {field.required && (
                ...
                {field.type === "textarea" ? (
                  ...
                  <div
                    className={
                      isDurationSection ? "flex items-start gap-4" : ""
                    }
                  >
                    {isDurationSection ? (
                      <div className="flex gap-2">
                        <select
                          value={getMonthValue(watchedValues[field.key])}
                          onChange={e =>
                            handleMonthChange(field.key, e.target.value)
                          }
                          disabled={field.key === "endDate" && isOngoing}
                          className={`p-2 border rounded-md ${
                            errors[field.key] ||
                            (field.key === "endDate" && errors.isOngoing)
                              ? "border-red-500 focus:border-red-500"
                              : "border-gray-300 focus:border-blue-500"
                          } focus:outline-none focus:ring-2 focus:ring-blue-500/20 ${
                            field.key === "endDate" && isOngoing
                              ? "bg-gray-100"
                              : ""
                          }`}
                        >
                        ...
                        <select
                          value={getYearValue(watchedValues[field.key])}
                          onChange={e =>
                            handleYearChange(field.key, e.target.value)
                          }
                          disabled={field.key === "endDate" && isOngoing}
                          className={`p-2 border rounded-md ${
                            errors[field.key] ||
                      ...
                      <label className="flex items-center gap-2 mt-2">
                        <input
                          ...
                          onChange={e => {
                            ...
                            setValue("isOngoing", checked);
                            if (checked) {
                              setValue("endDate", null);
                              clearErrors("endDate");
                            } else {
                              setValue("endDate", watchedValues.endDate);
                            }
                          }}
                          className="w-4 h-4"
                        />
