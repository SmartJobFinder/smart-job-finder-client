...
export default function CandidateSkillModal({
  ...
  const fetchIndustries = async categoryId => {
    ...
    setIsLoading(true);
    try {
      const response = await api.get(`/category/children-by-id/${categoryId}`);
  ...
  useEffect(() => {
    if (
      ...
      watchedValues.categoryId !== initialData.parentCategoryId?.toString()
    ) {
        ...
        <FormProvider {...methods}>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              {errors.skillId && (
                <p className="text-xs text-red-500">{errors.skillId.message}</p>
              )}
            </div>
