    ...
    const performSearch = async () => {
      try {
        if (name && categoryIds.length > 0) {
          // Tìm kiếm kết hợp
          await searchCompanies({ name, categoryIds }, page, size, sortParam);
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <option value="id,asc">{t`Default (A-Z)`}</option>
                  ...
                  <option value="locationCity,asc">{t`Location (A-Z)`}</option>
                  <option value="locationCity,desc">{t`Location (Z-A)`}</option>
            ...
            <div className="text-sm text-gray-600 mb-4">
              {t`Displaying`} {pagination.page * pagination.size + 1} -{" "}
              {Math.min(
                (pagination.page + 1) * pagination.size,
                pagination.totalElements
              )}{" "}
              {t`out of`} {pagination.totalElements} {t`companies`}
            </div>
          </div>
