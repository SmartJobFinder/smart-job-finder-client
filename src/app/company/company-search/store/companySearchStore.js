  ...
  getFilteredCompanies: () => {
    ...
    if (searchTerm.location) {
      ...
      filtered = filtered.filter(company => {
        ...
        if (company.locationCountry) {
          const companyCountry = normalizeString(company.locationCountry);
        ...
        if (fullLocation) {
          const normalizedFullLocation = normalizeString(fullLocation);
          if (
    ...
    if (filters.categoryIds.length > 0) {
      ...
      filtered = filtered.filter(company => {
        const matchById =
          company.categoryIds &&
          company.categoryIds.some(id => filters.categoryIds.includes(id));
        const matchByParent =
    ...
    if (filters.foundingYear !== "any") {
      ...
      filtered = filtered.filter(company => company.foundedYear === year);
    }
