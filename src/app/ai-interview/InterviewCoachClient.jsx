...
export default function InterviewCoachClient() {
  ...
  // ========= UI STATE =========
  const [showQuestionScript, setShowQuestionScript] = useState(true);
  ...
  const [answerInterview, { isLoading: submitting }] = useAnswerInterviewMutation();
  ...
  const stopRecordingAndSubmit = useCallback(async () => {
    ...
    try {
      if (!jobId || !sessionId || !currentQuestion) {
        throw new Error("Thiếu jobId/sessionId/question. Bấm Start lại giúp mình.");
      }
      ...
      const file = new File([blob], `answer.${ext}`, { type: blob.type || "audio/webm" });
      fd.append("file", file);
      ...
      const nextAudio = data.next_question_audio_url ?? data.nextQuestionAudioUrl ?? null;
      ...
      console.error(e);
      alert(`Submit answer failed: ${e?.data || e?.message || "unknown error"}`);
    }
  }, [answerInterview, currentQuestion, jobId, sessionId, safePlay, stopMic]);
  ...
  }, [isRecording, sessionId, startRecording, stopRecordingAndSubmit, submitting]);
  ...
  useEffect(() => {
    ...
    const onEnded = () => setIsQuestionPlaying(false);
    ...
    <div className="max-w-5xl mx-auto my-6 space-y-6">
      ...
      {jobLoading ? (
        ...
        <div className="text-sm text-red-600">
          Load job failed: {String(jobError?.data || jobError?.message || jobError)}
        </div>
      ) : (
      ...
      {!sessionId && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 flex flex-col items-center gap-4">
          <p className="text-sm text-gray-600 text-center max-w-xl">
            Bấm <b>Start interview</b> để AI tạo câu hỏi đầu tiên cho công việc này.
            Sau đó bạn thu âm câu trả lời, AI sẽ chấm điểm + feedback và đưa câu hỏi tiếp theo.
          </p>
