      ...
      <Dialog open={true} onClose={onClose} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="w-full max-w-lg p-6 bg-white shadow-xl rounded-xl">
            <p className="text-center">{t`Loading application details`}...</p>
          </Dialog.Panel>
        </div>
      </Dialog>
    );
        ...
        <Dialog.Panel className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl space-y-5 max-h-[90vh] overflow-y-auto scrollbar-hide">
          <div className="flex items-start justify-between">
            <div>
              ...
              {isReapply ? (
                ...
                <p className="text-sm text-gray-600">
                  Your information has been pre-filled. Please review and edit
                  if needed.
                </p>
              )}
            </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="p-4 space-y-4 border border-gray-300 rounded-lg">
              <div>
                <p className="mb-1 text-sm font-medium text-gray-700">CV:</p>
                {isReapply && applicationDetail?.cv && (
                  <div className="p-3 mb-3 rounded-lg bg-gray-50">
                    ...
                    <div className="flex gap-2">
                      <Button
                        type="button"
                        variant={keepCurrentCV ? "default" : "outline"}
                        size="sm"
                        onClick={() => handleCVSelection(true)}
                      >
                        {t`Keep Current CV`}
                      </Button>
                      <Button
                ...
                {(!isReapply || !keepCurrentCV) && (
                  <>
                    <Button
                      ...
                      disabled={isSubmitting}
                    >
                      {selectedFile ? t`Change CV` : t`Upload CV`}
                    </Button>
                    <input
                      type="file"
                      accept=".pdf,.doc,.docx"
            ...
            <div className="flex justify-end gap-2 pt-4 border-t">
              ...
              <Button
                ...
                disabled={isSubmitting || (isReapply && !isDirty)}
              >
                {isSubmitting
                  ? isReapply
                    ? t`Re-applying` + "..."
