    ...
    () =>
      guardOr(() => {
        ...
        if (isInactiveStatus || isExpired) {
          toast.error(t`This job is no longer accepting applications.`);
          return;
        }
    ...
    async () =>
      guardOr(async () => {
        if (attemptCount >= MAX_REAPPLY) {
          toast.error(t`You have reached the re-application limit (2 times).`);
          return;
        }
      ...
      <div className="flex flex-col-reverse w-full gap-6 md:flex-row">
        <div className="w-full md:w-[78%] flex flex-col gap-6">
          ...
          <div className="p-6 space-y-4 bg-white shadow-lg rounded-xl">
            <h1 className="text-3xl font-bold text-gray-800">{dj.title}</h1>

            <div className="flex flex-wrap gap-2 text-sm">
              <Pill icon={MapPin} className="text-blue-800 bg-blue-100">
                {formatList(dj.workType)}
              </Pill>
              <Pill icon={Layers} className="text-purple-800 bg-purple-100">
                {formatList(dj.category)}
              </Pill>
              <Pill icon={Briefcase} className="text-yellow-800 bg-yellow-100">
                {formatList(dj.level)}
              </Pill>
            </div>
            ...
            <div className="grid items-center grid-cols-1 md:grid-cols-10 gap-2 mt-4">
              <div className="col-span-8">
                {isLoadingApply ? (
                  ...
                  <div className="flex gap-2">
                    <Button
                      disabled={isExpired || reachedLimit || refreshingStatus} // disable khi hết hạn, đạt limit 2 lần, hoặc đang refetch
              ...
              <div className="relative flex items-center justify-center col-span-1">
                <button
                  ...
                  aria-label={liked ? t`Unsave job` : t`Save job`}
            ...
            {applied && showDetailModal && <ApplicationDetail jobId={dj.id} />}
          </div>
        ...
        <div className="w-full md:w-[22%] flex flex-col gap-4">
          ...
          <GeneralCard
            ...
            scamScore={dj?.scamScore}
          />
          <SkillsChips skills={Array.isArray(dj.skill) ? dj.skill : []} />
          <button
          ...
          <button
            ...
            title={
              isLoggedIn ? t`AI Interview Coaching` : t`Please log in to use AI`
            }
          >
